...
void flash_test(){
	uint32_t data[SECTOR_SIZE_IN_32BITS_WORDS];
	uint32_t i;
	for (i = 0; i < SECTOR_SIZE_IN_32BITS_WORDS; ++i) {
		data[i] = 0xAABBCCDD;
	}

	HAL_GPIO_WritePin(LED_FLASH_GPIO_Port, LED_FLASH_Pin, GPIO_PIN_SET);
	if (FLASH_OK != flash_write(0x080a0000, data, SECTOR_SIZE_IN_32BITS_WORDS)) {
	...
	if (FLASH_OK != flash_read(0x080a0000, data, SECTOR_SIZE_IN_32BITS_WORDS)) {
		HAL_GPIO_WritePin(LED_FAULT_GPIO_Port, LED_FAULT_Pin, GPIO_PIN_SET);
	}

	if (FLASH_OK != flash_write(0x080c0000, data, SECTOR_SIZE_IN_32BITS_WORDS)) {
		HAL_GPIO_WritePin(LED_FAULT_GPIO_Port, LED_FAULT_Pin, GPIO_PIN_SET);
	}
